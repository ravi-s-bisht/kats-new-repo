...
export async function POST(req: Request) {
  try {
    ...
    const [newMedication] = await db('medications')
      .insert({ user_id, medication_name, reminder_time: reminder_time });
...
export async function PUT(req: Request) {
  try {
    ...
    const reminderTimeUtc = moment(reminder_time).utc().format('YYYY-MM-DD HH:mm:ss');
